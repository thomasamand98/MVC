...
export class AttelageController {
  ...
  @Get('attelages')
  async getAttelages(@Query('page') page?: string, @Query('pageSize') pageSize?: string) {
    return this.attelageService.getAttelages(page ? Number(page) : undefined, pageSize ? Number(pageSize) : undefined);
  }
